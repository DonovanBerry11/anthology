...
| `onboarding.html` | Post-signup questionnaire — **6 steps**: country of residence (searchable dropdown) → city (searchable dropdown) → country of origin (searchable dropdown) → interests (checkboxes) → **content depth preference** (radio tiles: in-depth / summaries / both / unsure) → additional context (optional textarea). Saves structured `user_metadata` (including `content_depth`) to Supabase; appends depth preference sentence to `reading_interests` composite string; sets `onboarding_complete: true`; fires two fire-and-forget fetches to `/api/bootstrap` and `/api/first-dispatch` (Vercel proxy routes — avoids mixed-content block; see `vercel.json`); redirect is inside the async IIFE so fetches are dispatched before navigation begins. `SearchableSelect` dropdowns use a `mousedown` listener on the wrapper div (not `click` on the input) — fires before blur on other elements, preventing race conditions. `_open()` toggles: if already open, closes instead of double-opening. |
...
| `vercel.json` | `cleanUrls`, `trailingSlash: false`. Rewrites: (1) `/api/bootstrap` and `/api/first-dispatch` → `http://159.65.80.203:5050/…` — server-side proxy so onboarding fetches go over HTTPS from the browser to Vercel, avoiding mixed-content blocks; (2) `/users/:id/{section}/:slug` → `…/:slug.html` for all five content types: `essays`, `notes`, `dispatches`, `uk-politics`, `us-sports`. If a new content type is added under `users/`, add a matching rewrite rule or clean URLs for that type will 404. |
...
6. Fires two fire-and-forget `fetch` calls: `POST /api/bootstrap` and `POST /api/first-dispatch` — both proxied by Vercel to `http://159.65.80.203:5050/…` (server-side proxy avoids mixed-content block on the HTTPS site). Calls are made inside an async IIFE; redirect to `/` is the final line inside the IIFE, ensuring fetches are dispatched before navigation begins.
7. Redirects to `/`.
