...
def build_orientation(user_id, email, display_name, reading_interests,
    ...
    if has_structured:
        ...
        _depth_labels = {
            "in-depth":  "In-depth analysis and arguments",
            "summaries": "Simple summaries",
            "both":      "Both analysis and summaries",
            "unsure":    "Not sure / no preference stated",
        }
        depth_label = _depth_labels.get(content_depth, "")
        depth_line  = depth_label if depth_label else "(not stated)"
        content_depth_block = f"\n**Content depth preference:** {depth_line}"
...
**Origin:** {origin_line}
**Declared interests:** {interests_line}{content_depth_block}{additional_block}
...
interests are submitted and engagement data accumulates.*"""
    ...
    # Build Format and Depth section — pre-populate from stated preference if available
    _depth_labels_full = {
        "in-depth":  "In-depth analysis and arguments",
        "summaries": "Simple summaries",
        "both":      "Both analysis and summaries",
    }
    if content_depth and content_depth in _depth_labels_full:
        _depth_stated = _depth_labels_full[content_depth]
        format_depth_section = f"""\
## Format and Depth Preferences

**Stated preference (from onboarding):** {_depth_stated}

*Stated at signup — treated as a prior. Will be recalibrated against engagement \
signals as content accumulates. Whether longer analytical pieces are read in full \
or bounced determines whether stated preference aligns with actual reading behaviour.*"""
    else:
        format_depth_section = """\
## Format and Depth Preferences
...
def build_voice_md(user_id, today, interests=None, additional_info="",
                   country_of_residence="", city="", content_depth=""):
    """
...
**State:** `{"populated" if content_depth and content_depth != "unsure" else "unknown"}`

**Depth preference** *(in-depth analysis / summaries / both)*:
> {"In-depth analysis and arguments" if content_depth == "in-depth" else "Simple summaries" if content_depth == "summaries" else "Both analysis and summaries" if content_depth == "both" else "unknown"}
...
**Fields updated:** register (partial), vocabulary (partial), lead framing (partial) — inferred from interests[]; depth preference from content_depth; all other fields marked unknown
**Notes:** Generated by bootstrap_orientation.py immediately after orientation.md was written.
Interests: {", ".join(interests) if interests else "(none stated)"}. Content depth preference: {content_depth if content_depth else "not stated"}. Additional info: {"present" if has_additional else "not provided"}.
...
def main():
    ...
    content_depth        = metadata.get("content_depth", "")
    ...
    if content_depth:
        print(f"  depth pref:  {content_depth}")
